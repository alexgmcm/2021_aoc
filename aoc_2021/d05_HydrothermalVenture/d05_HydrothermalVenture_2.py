...
def mark_points(filt_input):
    ...
    for line in filt_input:
        if line[0][0] == line[1][0]:
            ...
            leftmost_point = list(filter(lambda x: x[1]==min(line[0][1],line[1][1]),line))[0]
            rightmost_point = list(filter(lambda x: x[1]==max(line[0][1],line[1][1]),line))[0]
            #diagonal is going up or down?
            if leftmost_point[0] <= rightmost_point[0]:
                #going up or single point
                cur_point = list(leftmost_point)
                while cur_point != list(rightmost_point):
                    point=str(cur_point)
                    if point in marked_points:
                        marked_points[point] += 1
                    else:
...
def main():
    ...
    marked_points = mark_points(proc_input)
